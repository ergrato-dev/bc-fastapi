# 🌐 Webgrafía - Semana 14

## Documentación Oficial

### Rate Limiting
- [slowapi Documentation](https://slowapi.readthedocs.io/)
- [Redis Rate Limiting](https://redis.io/glossary/rate-limiting/)
- [limits Library](https://limits.readthedocs.io/)

### Seguridad
- [OWASP API Security Top 10](https://owasp.org/API-Security/)
- [OWASP Secure Headers Project](https://owasp.org/www-project-secure-headers/)
- [MDN - HTTP Headers](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers)
- [Content Security Policy (CSP)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP)

### Logging
- [structlog Documentation](https://www.structlog.org/)
- [Python Logging HOWTO](https://docs.python.org/3/howto/logging.html)
- [Logging Cookbook](https://docs.python.org/3/howto/logging-cookbook.html)

### Métricas y Monitoreo
- [Prometheus Documentation](https://prometheus.io/docs/)
- [prometheus-fastapi-instrumentator](https://github.com/trallnag/prometheus-fastapi-instrumentator)
- [Grafana Documentation](https://grafana.com/docs/)
- [OpenTelemetry Python](https://opentelemetry.io/docs/languages/python/)

### Health Checks
- [Kubernetes Probes](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/)
- [Health Check API Pattern](https://microservices.io/patterns/observability/health-check-api.html)

---

## Artículos Técnicos

### Rate Limiting
- [Rate Limiting Strategies and Techniques](https://cloud.google.com/architecture/rate-limiting-strategies-techniques)
- [How to Design a Rate Limiter](https://www.educative.io/blog/rate-limiting-system-design)
- [Token Bucket Algorithm](https://en.wikipedia.org/wiki/Token_bucket)
- [Sliding Window Rate Limiting](https://blog.cloudflare.com/counting-things-a-lot-of-different-things/)

### Seguridad
- [Security Headers Quick Reference](https://web.dev/security-headers/)
- [CORS Explained](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS)
- [JWT Best Practices](https://auth0.com/blog/jwt-security-best-practices/)
- [API Security Checklist](https://github.com/shieldfy/API-Security-Checklist)

### Logging
- [Structured Logging Best Practices](https://www.structlog.org/en/stable/why.html)
- [The Twelve-Factor App - Logs](https://12factor.net/logs)
- [JSON Logging in Python](https://www.datadoghq.com/blog/python-logging-best-practices/)

### Observabilidad
- [Three Pillars of Observability](https://www.oreilly.com/library/view/distributed-systems-observability/9781492033424/ch04.html)
- [Prometheus Best Practices](https://prometheus.io/docs/practices/naming/)
- [Metrics Types Explained](https://prometheus.io/docs/concepts/metric_types/)
- [RED Method](https://grafana.com/blog/2018/08/02/the-red-method-how-to-instrument-your-services/)

---

## Herramientas Online

### Testing de Seguridad
- [Security Headers Scanner](https://securityheaders.com/)
- [Mozilla Observatory](https://observatory.mozilla.org/)
- [SSL Labs Test](https://www.ssllabs.com/ssltest/)

### Generadores
- [CSP Generator](https://report-uri.com/home/generate)
- [UUID Generator](https://www.uuidgenerator.net/)
- [JWT Debugger](https://jwt.io/)

### Monitoreo
- [Prometheus Playground](https://prometheus.io/docs/prometheus/latest/getting_started/)
- [Grafana Play](https://play.grafana.org/)

---

## Blogs Recomendados

- [Cloudflare Blog](https://blog.cloudflare.com/) - Rate limiting, CDN, Security
- [Netflix Tech Blog](https://netflixtechblog.com/) - Observability at scale
- [Datadog Blog](https://www.datadoghq.com/blog/) - Monitoring best practices
- [TestDriven.io](https://testdriven.io/blog/) - FastAPI tutorials
- [Real Python](https://realpython.com/) - Python best practices

---

## GitHub Repositories

### Rate Limiting
- [slowapi](https://github.com/laurentS/slowapi)
- [limits](https://github.com/alisaifee/limits)
- [flask-limiter](https://github.com/alisaifee/flask-limiter)

### Seguridad
- [secure](https://github.com/TypeError/secure)
- [python-jose](https://github.com/mpdavis/python-jose)

### Observabilidad
- [structlog](https://github.com/hynek/structlog)
- [prometheus-fastapi-instrumentator](https://github.com/trallnag/prometheus-fastapi-instrumentator)
- [opentelemetry-python](https://github.com/open-telemetry/opentelemetry-python)

---

## 🔖 Quick Reference

| Tema | Recurso Principal |
|------|------------------|
| Rate Limiting | slowapi docs |
| Security Headers | OWASP Secure Headers |
| CORS | MDN CORS Guide |
| Logging | structlog docs |
| Metrics | Prometheus docs |
| Health Checks | Kubernetes probes |
